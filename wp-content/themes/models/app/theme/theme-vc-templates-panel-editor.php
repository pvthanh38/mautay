...
/**
 * Class Rella_Vc_Templates_Panel_Editor
 * @since 1.0
 */
class Rella_Vc_Templates_Panel_Editor {
	/**
	 ...
	 * @var bool
	 */
	protected $rella_templates = false;
	/**
	 * @since 4.4
...
//		$enable_boo_universe = rella_helper()->get_option( 'enable-boo-universe' );
//		if( ! $enable_boo_universe ) {
//			return;
//		}
	...
	public function addTemplatesTab( $data ) {
		$newCategory = array(
			'category'        => 'rella_templates',
			'category_name'   => esc_html__( 'Theme Templates', 'vtm' ),
			'category_weight' => 1,
	...
	protected function get_template_categories() {
		...
		$categories = array(
			'all'         => esc_html__( 'All', 'vtm' ),
			'banner'      => esc_html__( 'Banners', 'vtm' ),
			'post'      => esc_html__( 'Posts', 'vtm' ),
	...
	public function renderTemplateBlock( $category ) {
		...
		if ( 'rella_templates' === $category['category'] ) {

			$category['output'] = '<div class="vc_col-md-2 rella-sorting-container">';
			$category['output'] .= $this->get_template_categories();
			$category['output'] .= '</div>';


			$category['output'] .= '
			<div class="vc_column vc_col-sm-12 rella-templates-container">
		...
		//file_put_contents(ABSPATH.'/categories.txt',serialize($category));
	...
	function renderTemplateWindow( $template_name, $template_data ) {
		...
		if ( 'rella_templates' === $template_data['type'] ) {
			return $this->renderTemplateWindowRellaTemplates( $template_name, $template_data );
		}
	 ...
	 * @return string
	 */
	public function renderTemplateWindowRellaTemplates( $template_name, $template_data ) {
		ob_start();
		...
		$preview_template_title = esc_attr( 'Preview template', 'boo' );
		$add_template_title = esc_attr( 'Add template', 'boo' );
...
HTML;
	...
	public function renderBackendTemplate() {
		...
		if ( ! isset( $template_id, $template_type ) || '' === $template_id || '' === $template_type ) {
			die( 'Error: Vc_Rella_Templates::renderBackendTemplate:1' );
		}
	 ...
	 * @param $templates
	 *
	 * vc_filter: vc_load_rella_templates_limit_total - total items to show
	 *
	 * @return array
	 */
	...
	public function getAllTemplates() {
		...
		$data = array();
		$rella_templates = $this->getTemplates();
		$category_templates = array();
		foreach ( $rella_templates as $template_id => $template_data ) {
			$category_templates[] = array(
				'unique_id' => $template_id,
				'name' => $template_data['name'],
				'type' => 'rella_templates',
			...
			//file_put_contents(ABSPATH.'/getAll.txt', serialize($data));
		}
	 ...
	 * To modify you should use add_filter('vc_load_rella_templates','your_custom_function');
	 ...
	 *          ...
	 *      );
	 * Also see filters 'vc_load_rella_templates_panels' and 'vc_load_rella_templates_welcome_block' to modify
	 * templates in panels tab and/or in welcome block. vc_filter: vc_load_rella_templates - filter to override
	...
	public function loadDefaultTemplates() {
		...
		if ( ! is_array( $this->rella_templates ) ) {
			$this->rella_templates = $this->allTemplates();
		}

		return $this->rella_templates;
	}
	...
	public function getDefaultTemplate( $template_index ) {
		...
		if ( ! is_numeric( $template_index ) || ! is_array( $this->rella_templates ) || ! isset( $this->rella_templates[ $template_index ] ) ) {
			return false;
		}

		return $this->rella_templates[ $template_index ];
	}
	...
	public function addDefaultTemplates( $data ) {
		if ( is_array( $data ) && ! empty( $data ) && isset( $data['name'], $data['content'] ) ) {
			if ( ! is_array( $this->rella_templates ) ) {
				$this->rella_templates = array();
			}
			$this->rella_templates[] = $data;
	...
	public function renderTemplateListItem( $template ) {
		$name = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'boo' ) );
...
HTML;
		$output .= '<div class="rella-template-preview"><img src="' . esc_url( $template_image ) . '" alt="' . esc_attr( $name ) . '" width="300" height="200" /></div>';
		$output .= apply_filters( 'vc_templates_render_template', $name, $template );
		$output .= '<span class="rella-template-categories">' . esc_html( $template_sort_name ) . '</span>';
		$output .= <<<HTML
						</div>
